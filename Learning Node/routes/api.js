...
router.get('/techies',function(req,res,next){
  /* Techie.find({}).then(function(techies){
   ...
   Techie.aggregate([{ $geoNear: { near: {type:"point",coordinates:[parseFloat(req.query.lng),parseFloat(req.query.lat)]},spherical :true, maxDistance:100000,distanceField: "dist.calculated",spherical:true}}]).then(function(techies){
    res.send(techies);
